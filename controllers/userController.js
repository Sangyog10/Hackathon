  ...
  try {
    ...
    if (
      ...
      throw new CustomError(
        "Invalid input. Latitude, longitude, and distance must be valid numbers.",
        StatusCodes.BAD_REQUEST
      );
    }

    const isInZone = is_within(
      parsedLat1,
      parsedLon1,
      parsedLat2,
      parsedLon2,
...
const calculateRewardPoints = async (req, res) => {
  ...
  if (isPostedByAdmin) {
    ...
    res
      ...
      .json({ msg: "Reward points added", points: user.points });
  } else {
    user.totalTimeInZone = 0;
    await user.save();
...
module.exports = { findUserInLocation, calculateRewardPoints, userDetails };
